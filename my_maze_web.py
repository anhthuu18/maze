...
# Lưu trữ các điểm và trạng thái
...
# Kiểm tra xem người dùng đã chọn đủ 2 điểm chưa
if len(st.session_state["points"]) == 2:
    x1, y1 = st.session_state["points"][0]
    x2, y2 = st.session_state["points"][1]
    
    # Hiển thị nút để sinh đường đi
    if st.button('Generate Path'):
        try:
            # Kiểm tra xem điểm bắt đầu và kết thúc có phải là các ô hợp lệ không
            if MAP[y1][x1] == '#' or MAP[y2][x2] == '#':
                st.error("Điểm bắt đầu hoặc điểm kết thúc không hợp lệ!")
                st.stop()
